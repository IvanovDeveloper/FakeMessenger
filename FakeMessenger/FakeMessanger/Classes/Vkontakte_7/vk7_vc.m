...
#pragma mark Setup View
    ...
    self.view.backgroundColor = RGBa(237.0f, 243.0f, 250.0f, 1.0f);
...
#pragma mark Setup TabBar
    ...
    [ self.navigationController.navigationBar setBackgroundImage:[ UIImage imageNamed:@"vk7_tb_TapBarBackgound"] forBarMetrics:UIBarMetricsDefault ];
...
//Recipient name
    ...
    self.nameUser.font = [ UIFont fontWithName:@"Helvetica-Bold" size:16 ];
    ...
    for( DB_vk7 *message in self.controller.fetchedObjects )
    {
...
//Left Button
    ...
    UIImage *backBtnImage = [ UIImage imageNamed:@"vk7_tb_BackButton" ];
    UIView *containingView = [[UIView alloc] init ];
    
    if ( is_ios_7() )
        containingView.frame = CGRectMake(0, 0, backBtnImage.size.width - 8.f, backBtnImage.size.height);
    else
...
//RightButton
    ...
    NSString *STR = [NSHomeDirectory() stringByAppendingPathComponent:@"Library/Caches/vk7_image.png"];
    ...
    self.avatarButton.layer.cornerRadius = 17.0f;
...
{
    int offsetDate = 0;
    DB_vk7 *message1 = [ self.controller objectAtIndexPath:indexPath ];
    if ( indexPath.row == 0)
    {
    ...
    else
    {
        ...
        DB_vk7 *message2 = [ self.controller objectAtIndexPath:indexPathBefore ];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    NSString *CellInt = @"CellIndentify";
     vk7_message_cell *cell = [ tableView dequeueReusableCellWithIdentifier:CellInt ];
    
    if( cell == nil )
    {
        cell = [ [ vk7_message_cell alloc ] initWithStyle:UITableViewCellStyleDefault
                                         reuseIdentifier:CellInt ];
    }
    
    if ( indexPath.row == 0)
    {
        DB_vk7 *message = [ self.controller objectAtIndexPath:indexPath ];
...
{
    DB_vk7 *message = self.controller.fetchedObjects[ indexPath.row ];
...
{
    DB_vk7 *message1 = self.controller.fetchedObjects[  sourceIndexPath.row ];
    DB_vk7 *message2 = self.controller.fetchedObjects[ destinationIndexPath.row ];
...
{
    DB_vk7 *message = self.controller.fetchedObjects[ indexPath.row ];
    vk7_settings_vc  *newViewPostMessage = [[ vk7_settings_vc alloc ] initWithInfoMessage:message
...
//Add to CoreData
                    for(DB_vk7 *item in self.controller.fetchedObjects)
                    {
...
{
    UIImageView *panelFake = [ [ UIImageView alloc ] initWithImage:[ UIImage imageNamed:@"vk7_FakePanel_RUS" ] ];
    if ( is_4_inch() )
...
{
    vk7_settings_vc *newViewPostMessage = [ [ vk7_settings_vc alloc ] initWithRecipientName:self.nameUser.text andCarrierName:self.carrierField.text ];
    [ self.navigationController pushViewController:newViewPostMessage animated:YES ];
}
...
- (void)sortDataBase
{
    ...
    NSFetchRequest *request = [ NSFetchRequest fetchRequestWithEntityName:@"DB_vk7" ];
    request.fetchBatchSize = 10;
    NSSortDescriptor *sortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"d_timestamp" ascending:YES];
    NSArray *sortDescriptors = @[ sortDescriptor ];
    [ request setSortDescriptors:sortDescriptors ];
    request.predicate = [ NSPredicate predicateWithFormat:@"s_type_message == %@", @"VK7" ];
